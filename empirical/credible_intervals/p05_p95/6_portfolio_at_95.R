...
a  <- apply(reslf$r[ind,2:(dm+1)],2,p95)
b  <- apply(reslf$r[ind,(dm+2):(2*dm+1)],2,p95)
nu <- p95(reslf$r[ind,1])
iota    = rep(1,dm)
Oiota   = Outer(iota,iota)

# for DCC-HEAVY-t model
Rh      = array(NA,c(dm, dm, nn+K))
nuh     <- p95(resH$r[ind,1])
ah      <- p95(resH$r[ind,2])
bh      <- p95(resH$r[ind,3])
Pbar = Reduce('+',Sigma[1:nn])/nn

# for High Frequency (CAW) model
nux  = p95(reshf$r[ind,1])
b1   = apply(reshf$r[ind,2:(dm+1)],2,p95)
b2   = apply(reshf$r[ind,(dm+2):(2*dm+1)],2,p95)
...
wj1 = apply(ws_jore1,2,p95)
wgw = apply(ws_gew,2,p95)
...
save.image(file = 'empirical/temp/FX_portfolio_at_p95.Rdata')
