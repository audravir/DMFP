...
ws_gmvr.oracle = ws_gmvr
ws_CVAR05r.oracle = ws_CVAR05r
ws_CVAR10r.oracle = ws_CVAR10r

all_objects <- ls()
keep_objects <- c("ws_gmvr.oracle", "ws_CVAR05r.oracle", "ws_CVAR10r.oracle")
rm(list = setdiff(all_objects, keep_objects))

load('empirical/temp/FX_portfolio_at_median_0320.Rdata')

# ADD one more model - ORACLE and the weights

models[7] = 'Oracle'

ws_gmvr    <- abind(ws_gmvr, ws_gmvr.oracle, along = 1)
ws_CVAR05r <- abind(ws_CVAR05r, ws_CVAR05r.oracle, along = 1)
ws_CVAR10r <- abind(ws_CVAR10r, ws_CVAR10r.oracle, along = 1)
...
# how many assets are involved in the restricted portfolio
par(mfrow=c(3,1))
plot(apply((ws_gmvr[1,,]>0.01),1,sum),type='l',ylim=c(4,9))
plot(apply((ws_CVAR05r[1,,]>0.01),1,sum),type='l',ylim=c(4,9))
plot(apply((ws_CVAR10r[1,,]>0.01),1,sum),type='l',ylim=c(4,9))
...
pret      = co = to = portsd = array(NA,dim=c(1,K))

for(j in 1:length(all.ws)){
    for(t in 1:K){
      pret[1,t]   = sum(all.ws[[j]][1,t,]*rets[nn+t,])
      portsd[1,t] = sqrt(t(all.ws[[j]][1,t,])%*%RCov[,,nn+t]%*%all.ws[[j]][1,t,])
      co[1,t]     = (sum((all.ws[[j]][1,t,])^2))^(1/2)

      if(t<K)  to[1,t]   = sum(abs(all.ws[[j]][1,t+1,]-all.ws[[j]][1,t,]*((1+rets[nn+t,])/(1+pret[1,t]))))
    }
  all.prets[[j]] = pret
  all.co[[j]]    = co
  all.to[[j]]    = to
...
names(all.prets) = names(all.co) = names(all.to) = names(all.sd) =  c('gmvr','CVAR05r','CVAR10r')
