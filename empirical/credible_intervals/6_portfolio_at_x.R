...
ws_gmvr = ws_CVAR05r = ws_CVAR10r = array(NA,c(length(models),K,dm,postM))
...
for(m in 1:postM){
  ...
  a  <- reslf$r[ind2,2:(dm+1)][m,]
  b  <- reslf$r[ind2,(dm+2):(2*dm+1)][m,]
  nu <- reslf$r[ind2,1][m]
  ...
  nux  = reshf$r[ind,1][m]
  b1   = reshf$r[ind,2:(dm+1)][m,]
  b2   = reshf$r[ind,(dm+2):(2*dm+1)][m,]
  ...
  for(t in 2:(nn+K)){
    ...
    if(t>nn){
      ...
      ws_gmvr[4,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.10, wmin = 0, wmax = 1)
      ws_CVAR10r[4,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.05, wmin = 0, wmax = 1)
      ws_CVAR05r[4,t-nn,,m]=c(res)
      ...
      res <- minvar(SS, wmin = 0, wmax = 1)
      ws_gmvr[5,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.10, wmin = 0, wmax = 1)
      ws_CVAR10r[5,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.05, wmin = 0, wmax = 1)
      ws_CVAR05r[5,t-nn,,m]=c(res)
      ...
      res <- minvar(SS, wmin = 0, wmax = 1)
      ws_gmvr[6,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.10, wmin = 0, wmax = 1)
      ws_CVAR10r[6,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.05, wmin = 0, wmax = 1)
      ws_CVAR05r[6,t-nn,,m]=c(res)
      ...
      #
      if(ws_jore1[ind3,t-nn][m]>US[1,t-nn]) selected = sample_retsxm else selected = sample_retsdcct
      ...
      res <- minvar(SS, wmin = 0, wmax = 1)
      ws_gmvr[1,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.10, wmin = 0, wmax = 1)
      ws_CVAR10r[1,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.05, wmin = 0, wmax = 1)
      ws_CVAR05r[1,t-nn,,m]=c(res)
      ...
      if(ws_gew[ind3,t-nn][m]>US[2,t-nn]) selected = sample_retsxm else selected = sample_retsdcct
      ...
      res <- minvar(SS, wmin = 0, wmax = 1)
      ws_gmvr[2,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.10, wmin = 0, wmax = 1)
      ws_CVAR10r[2,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.05, wmin = 0, wmax = 1)
      ws_CVAR05r[2,t-nn,,m]=c(res)
      ...
      res <- minvar(SS, wmin = 0, wmax = 1)
      ws_gmvr[3,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.10, wmin = 0, wmax = 1)
      ws_CVAR10r[3,t-nn,,m]=c(res)
      
      res <- minCVaR(tmp, 0.05, wmin = 0, wmax = 1)
      ws_CVAR05r[3,t-nn,,m]=c(res)
...
save.image(file = 'empirical/temp/FX_portfolio_at_x_10k.Rdata')
