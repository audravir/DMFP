...
for(m in 1:(bi+M)){
  ...
  # fac = sample(c(1,sqrt(5)),size=2,replace=TRUE,prob = c(0.9,0.1))
  ...
  repeat{
    ...
    if(counter >= 10){
      print(paste(cond1,cond2,cond3,cond4,'iter=',m,'aacp',round(mean(accB1[1:m]),2),'fac=',fac,sep=','))
      fac=fac/2
    }
  }
  for(t in 1:TT){
