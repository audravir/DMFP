...
# M3
RVsforc1_m3 = matrix(NA,ncol=ncol(RVs),nrow=oos)
for(i in 1:dm){
  x=as.xts(RVs[,i]^2,order.by=date)
  ForecastHAR = HARForecast(x, periods = c(1,5,35), nRoll =oos,
                            nAhead = 5, type = "HAR")
  RVsforc1_m3[,i] = ForecastHAR@forecast[1,]
}

par(mfrow=c(2,5))
for(i in 1:dm){
  plot(tail(RVs[,i]^2,oos),type='l')
...
ONEsa_mae  = matrix(NA,ncol=dm,nrow=3)
ONEsa_rmse = matrix(NA,ncol=dm,nrow=3)
...
for(i in 1:dm){
  ...
  em3ONEsa[,i] = RVsforc1_m3[,i] - tail(RVs[,i]^2,oos)
...
ONEsa = rbind(ONEsa_rmse/matrix(apply(ONEsa_rmse, 2, min),ncol=dm,nrow=3,byrow=TRUE)
,ONEsa_mae/matrix(apply(ONEsa_mae, 2, min),ncol=dm,nrow=3,byrow=TRUE)
)
